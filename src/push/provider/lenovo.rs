...
/// Lenovo Push Provider
pub struct LenovoProvider {
    client: Client,
    app_id: String,
    access_token: String,
    endpoint: String,
...
impl LenovoProvider {
    pub fn new(app_id: String, access_token: String, endpoint: Option<String>) -> Self {
        let endpoint = endpoint
            ...
            .map(str::trim)
            .filter(|v| !v.is_empty())
            .unwrap_or("https://push-api.lenovo.com")
            .trim_end_matches('/')
            .to_string();
        Self {
...
#[async_trait]
impl PushProvider for LenovoProvider {
    async fn send(&self, task: &PushTask) -> Result<()> {
        ...
        info!(
            "[LENOVO] Sending push: task_id={}, user_id={}, device_id={}",
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| ServerError::Internal(format!("Lenovo request failed: {}", e)))?;

        let status = response.status();
        let body = response.text().await.unwrap_or_default();
        if status.is_success() {
            info!("[LENOVO] Push sent successfully: task_id={}", task.task_id);
            Ok(())
        } else {
            error!(
                "[LENOVO] Push failed: task_id={}, status={}, error={}",
                task.task_id, status, body
            );
            Err(ServerError::Internal(format!(
                "Lenovo push failed: status={}, error={}",
                status, body
            )))
        }
    }
