...
/// OPPO / HeyTap Push Provider
pub struct OppoProvider {
    client: Client,
    app_id: String,
    access_token: String,
    endpoint: String,
...
impl OppoProvider {
    pub fn new(app_id: String, access_token: String, endpoint: Option<String>) -> Self {
        let endpoint = endpoint
            ...
            .map(str::trim)
            .filter(|v| !v.is_empty())
            .unwrap_or("https://api.push.oppomobile.com")
            .trim_end_matches('/')
            .to_string();
        Self {
    ...
    fn build_payload(&self, task: &PushTask) -> serde_json::Value {
        json!({
            "target_type": 2,
            "target_value": [task.push_token],
            "notification": {
                "app_message_id": task.task_id,
                "title": "新消息",
...
#[async_trait]
impl PushProvider for OppoProvider {
    async fn send(&self, task: &PushTask) -> Result<()> {
        let url = format!("{}/server/v1/message/notification/unicast", self.endpoint);
        let payload = self.build_payload(task);

        info!(
            "[OPPO] Sending push: task_id={}, user_id={}, device_id={}",
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| ServerError::Internal(format!("OPPO request failed: {}", e)))?;

        let status = response.status();
        let body = response.text().await.unwrap_or_default();
        if status.is_success() {
            info!("[OPPO] Push sent successfully: task_id={}", task.task_id);
            Ok(())
        } else {
            error!(
                "[OPPO] Push failed: task_id={}, status={}, error={}",
                task.task_id, status, body
            );
            Err(ServerError::Internal(format!(
                "OPPO push failed: status={}, error={}",
                status, body
            )))
        }
    }
